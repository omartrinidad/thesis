...
\node [diam, join] (is_closed) {
    $\lnot isClosed(r)$ or $conf_{pca}(r) < minC$
};
\node [proc, join] () {
    $parents$ = $parentsOfRule(r, out)$
};
\node [diam_small, join] (is_empty) {
    $\lnot$ parents.isEmpty()
};
\node [proc_small, join] () {
    $r_p$ = parents.dequeue()
};
\node [diam_small, join] (check_confidence) {
...
\path (is_closed.west) to node [near start, yshift=1em] {yes} (c0); 
\draw [*->] (is_closed.west) -- (c0) -- (c2) |- (false);

\path (is_empty.east) to node [near start, yshift=1em] {no} (c1); 
\draw [o->] (is_empty.east) -- (c1) -- (c3) |- (true);

\path (check_confidence.west) to node [near start, yshift=1em] {yes} (c2); 
\draw [*->] (check_confidence.west) -- (c2);

\path (check_confidence.east) to node [near start, yshift=1em] {no} (c3); 
